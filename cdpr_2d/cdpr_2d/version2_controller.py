...
class Version2Controller(Node):
    ...
    def target_coordinates_callback(self, msg: PoseStamped):
        ...
        self.get_logger().info(f"POSICIÓN FINAL RECIBIDA: ({self.target_x:.3f}, {self.target_y:.3f})")
    ...
    def control_loop(self):
        ...
        if self.moving and not self.objective_achieved:
            ...
            d1, d2, a1, a2 = self.calculate_pulley_movement(self.current_x, self.current_y, new_x, new_y)
    ...
    def publish_cable_parameters(self):
        if self.objective_achieved:
            return
        l1, l2, a1, a2 = self.calculate_cable_parameters(self.current_x, self.current_y)
        msg = Float32MultiArray()
        msg.data = [l1, l2, a1, a2]
        self.cable_parameters_publisher.publish(msg)
        self.get_logger().info(f"CABLES: L=({l1:.3f}, {l2:.3f}) m, ÁNGULOS=({a1:.2f}, {a2:.2f})°")
