...
class Version3Controller(Node):
    ...
    def __init__(self):
        ...
        self.coordinates_subscriber = self.create_subscription(
            PoseStamped, '/version3', self.pose_callback, qos_profile)
        ...
        if x == -1.0 and y == -1.0:
            if len(self.points) > 2:
                ...
                self.get_logger().info(
                    f'TRAYECTORIA OBTENIDA DE {len(self.points)} PUNTOS. INICIANDO MOVIMIENTO ...')
            else:
    ...
    def control_loop(self):
        ...
        if self.moving and not self.objective_achieved:
            ...
            if self.animation_progress >= 1.0:
                ...
                delta_l1, delta_l2, pulley1_angle, pulley2_angle = self.calculate_pulley_movement(
                ...
                return
            interpolated_x = self.initial_x + \
                (self.target_x - self.initial_x) * self.animation_progress
            interpolated_y = self.initial_y + \
                (self.target_y - self.initial_y) * self.animation_progress
            delta_l1, delta_l2, pulley1_angle, pulley2_angle = self.calculate_pulley_movement(
                self.current_x, self.current_y, interpolated_x, interpolated_y)
            self.left_pulley_position += pulley1_angle
            self.right_pulley_position += pulley2_angle
            self.publish_joint_states()
    ...
    def publish_effector_parameters(self):
        ...
        if self.moving:
            ...
            self.get_logger().info(
                ...
                f'{progress_percent:.1f}% - P. A. = ({self.current_x:.3f}, {self.current_y:.3f}), '
