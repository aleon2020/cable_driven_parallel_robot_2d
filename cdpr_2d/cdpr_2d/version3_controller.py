...
class Version3Controller(Node):
    ...
    def __init__(self):
        ...
        self.coordinates_subscriber = self.create_subscription(
            Path, '/version3', self.path_callback, qos_profile)
    ...
    def path_callback(self, msg: Path):
        ...
        if self.points_number < 2:
            self.get_logger().error('SE PIDEN AL MENOS DOS PUNTOS PARA DEFINIR UNA TRAYECTORIA')
            self.create_timer(0.05, self.shutdown_node)
            return
        self.get_logger().info(f'TRAYECTORIA RECIBIDA CON {self.points_number} \
