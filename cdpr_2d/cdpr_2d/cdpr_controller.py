...
class CDPRController(Node):
    ...
    def calculate_cable_parameters(self, x, y):
        ...
        left_len = math.sqrt((left_connection_x - left_pulley_x)**2 + (left_connection_y - left_pulley_y)**2)
        right_len = math.sqrt((right_connection_x - right_pulley_x)**2 + (right_connection_y - right_pulley_y)**2)
        left_ang = math.degrees(math.atan2(left_connection_x - left_pulley_x, left_pulley_y - left_connection_y))
        right_ang = math.degrees(math.atan2(right_pulley_x - right_connection_x, right_pulley_y - right_connection_y))
    ...
    def handle_single_point(self):
        ...
        delta_l1, delta_l2, p1_ang, p2_ang = self.calculate_pulley_movement(self.plane_width/2.0, self.plane_height/2.0, self.current_x, self.current_y)
        path_msg = Path()
        ps = PoseStamped()
        ...
        print(f"• ÁNGULOS DE GIRO (RADIANES): P1: {p1_ang:+.3f} rad, P2: {p2_ang:+.3f} rad")
        print("=" * 100)
    ...
    def start_two_point_trajectory(self):
        ...
        self.left_pulley_position = 0.0
        self.right_pulley_position = 0.0
        l1, l2, a1, a2 = self.calculate_cable_parameters(self.target_x, self.target_y)
        l1_init, l2_init, a1_init, a2_init = self.calculate_cable_parameters(self.initial_x, self.initial_y)
        ...
        self.get_logger().info(f"NUEVA TRAYECTORIA: ({self.initial_x:.3f}, {self.initial_y:.3f}) → ({self.target_x:.3f}, {self.target_y:.3f})")
    ...
    def start_multi_point_trajectory(self):
        ...
        self.get_logger().info(f"TRAYECTORIA CREADA: {self.points_number} puntos, {len(self.segment_target_params)} segmentos.")
    ...
    def control_loop(self):
        ...
        if self.moving and not self.objective_achieved:
            ...
            if self.animation_progress >= 1.0:
                ...
                dl1, dl2, p1_ang, p2_ang = self.calculate_pulley_movement(self.initial_x, self.initial_y, self.current_x, self.current_y)
            ...
            dl1, dl2, p1_ang, p2_ang = self.calculate_pulley_movement(self.current_x, self.current_y, new_x, new_y)
            self.left_pulley_position += p1_ang
            self.right_pulley_position += p2_ang
            self.publish_joint_states()
    ...
    def publish_cable_parameters(self):
        ...
        self.get_logger().info(f"CABLES: L=({L1:.3f}, {L2:.3f}) m, ÁNGULOS=({Q1:.2f}, {Q2:.2f})°")
    ...
    def publish_pulley_parameters(self):
        ...
        if self.mode == 'multi':
            self.get_logger().info(f"POLEAS: SEGMENTO [{self.segment_index+1}/{max(1,self.points_number-1)}] DIF=({dl1:+.3f} m, {dl2:+.3f} m), ÁNGULOS=({pa1:+.3f} rad, {pa2:+.3f} rad)")
        else:
            self.get_logger().info(f"POLEAS: DIF=({dl1:+.3f} m, {dl2:+.3f} m), ÁNGULOS=({pa1:+.3f} rad, {pa2:+.3f} rad)")
