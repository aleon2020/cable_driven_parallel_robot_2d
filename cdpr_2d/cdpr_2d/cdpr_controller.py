...
# TERMINAL 6: ros2 topic pub --once /cdpr geometry_msgs/msg/PoseStamped
# "{header: {frame_id: 'world'}, pose: {position: {x: -1.0, y: -1.0, z:
# 0.0}}}"
...
class CDPRController(Node):
    ...
    def __init__(self):
        ...
        self.coordinates_subscriber = self.create_subscription(
            PoseStamped, '/cdpr', self.pose_callback, qos_profile)
    ...
    def pose_callback(self, msg: PoseStamped):
        ...
        self.get_logger().info(
            f"PUNTO {len(self.points)} RECIBIDO: ({x:.3f}, {y:.3f})")
    ...
    def handle_single_point(self):
        ...
        print(
            f"• ÁNGULOS DE GIRO (RADIANES): P1: {p1_ang:+.3f} rad, P2: {p2_ang:+.3f} rad")
        print("=" * 100)
    ...
    def start_multi_point_trajectory(self):
        ...
        self.get_logger().info(
            f"TRAYECTORIA CREADA: {
                self.points_number} puntos, {
                len(
                    self.segment_target_params)} segmentos.")
    ...
    def publish_pulley_parameters(self):
        ...
        if self.mode == 'multi':
            self.get_logger().info(f"POLEAS: SEGMENTO [{self.segment_index + 1}/{max(1,
                                                                                     self.points_number - 1)}] DIF=({dl1:+.3f} m, {dl2:+.3f} m), ÁNGULOS=({pa1:+.3f} rad, {pa2:+.3f} rad)")
        else:
            self.get_logger().info(
                f"POLEAS: DIF=({dl1:+.3f} m, {dl2:+.3f} m), ÁNGULOS=({pa1:+.3f} rad, {pa2:+.3f} rad)")
